...
def dynamic_batching_shard(
    ...
    """Sort sequences by length, distribute across DP ranks, and group into micro-batches
    respecting a token budget per micro-batch.

    Args:
        origin_batch: The input batch of sequences.
        dp_size: Number of data-parallel ranks.
        max_tokens_per_microbatch: Maximum tokens allowed in one micro-batch.
        sequence_length_round: Round up sequence lengths to multiples of this value.
        pipeline_model_parallel_size: Pipeline parallel size (for Megatron; default 1).
        virtual_pipeline_model_parallel_size: Virtual PP size (for Megatron; default None).
        log_prefix: Prefix for metric keys.

    Returns:
        A tuple of (reordered DataProto with micro-batch metadata, metrics dict).
    """
    ...
    for shard_indice in range(shard_size):
        ...
        for shard, seq_lens_shard in zip(aggregated_shards, seq_len_of_shard):
            seq_len = seq_lens_shard[shard_indice]
    ...
    if pipeline_model_parallel_size > 1 and virtual_pipeline_model_parallel_size:
        ...
        assert pipeline_model_parallel_size <= shard_size, (
            f"The pipeline_model_size: {pipeline_model_parallel_size} should not be greater than "
            f"num_seqs in one dp_rank"
        )
        assert padded_num_micro_batches <= shard_size
        num_micro_batches_needed = padded_num_micro_batches - num_micro_batches

        splittable_mbs = [
            i for i in range(num_micro_batches)
            if (global_micro_batch_indices[i][1] - global_micro_batch_indices[i][0]) > 1
        ]
        splittable_mbs.sort(
            key=lambda x: (global_micro_batch_indices[x][1] - global_micro_batch_indices[x][0])
            * global_micro_batch_lengths[x],
            reverse=True,
        )
        ...
        global_micro_batch_indices = [
            global_micro_batch_indices[i] for i in range(num_micro_batches) if i not in dropped_mbs
        ]
        global_micro_batch_lengths = [
            global_micro_batch_lengths[i] for i in range(num_micro_batches) if i not in dropped_mbs
        ]
    ...
    valid_tokens = sum(seq_lens_sort)
    actual_tokens_origin = batch_size * attention_mask.shape[-1]
    actual_tokens = total_tokens * dp_size
    removed_padding_tokens = actual_tokens_origin - actual_tokens
    removed_padding_ratio = removed_padding_tokens / actual_tokens_origin if actual_tokens_origin > 0 else 0
...
def make_mini_batch_iter_for_dynamic_batching(
    ...
    """Group previously created global micro batches into mini batches based on
...
def make_micro_batch_iter_for_dynamic_batching(mini_batch: DataProto) -> Iterator[DataProto]:
    """Yield individual micro-batches from a mini-batch, truncating sequences to
    their pre-computed lengths to remove unnecessary padding.

    Called from the strategy layer to split a mini-batch into individual
    micro-batches for forward/backward passes.
    """
    ...
    for seqlen, (start_idx, end_idx) in zip(micro_batch_lengths, micro_batch_indices):
        ...
        for k in mini_batch.batch.keys():
            if (
                len(micro_batch.batch[k].shape) == len(input_ids_shape) or k == "position_ids"
            ) and micro_batch.batch[k].shape[-1] in (
                input_ids_shape[-1],
                input_ids_shape[-1] - 1,
            ):
