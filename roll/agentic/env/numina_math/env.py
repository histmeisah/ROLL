...
class NuminaMathEnv(BaseEnv):
    """
    Numina Math environment for mathematical reasoning with web search capabilities.
    """

    def __init__(self, config: NuminaMathEnvConfig = None):
        self.config = config or NuminaMathEnvConfig()
        super(NuminaMathEnv, self).__init__(self.config)
    ...
    def step(self, action: str):
        ...
        if self.step_count >= self.config.max_steps:
            terminated = False
            truncated = True
            reward += self.config.invalid_action_penalty  # Penalty for not finishing in time
    ...
    def _compute_final_reward(self, answer: str) -> Tuple[float, bool, float, Dict]:
        """
        Compute final reward for answer using math-specific reward computation
        ...
        try:
            ...
            # Use math-specific reward computation
            ...
            reward_info = {
                ...
                "math_grading_info": result.get("math_grading_info", {})
            }

            return final_reward, success, raw_score, reward_info

        except Exception as e:
            # Fallback to simple matching if math reward fails
            print(f"Math reward computation failed: {e}, falling back to simple matching")
...
if __name__ == "__main__":
    ...
    # Simple demonstration of numina math environment
    print("Numina Math Environment Demo")
